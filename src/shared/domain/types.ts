...
export type FurnitureItem = {
  ...
  frieze?: boolean;
  /**
   * Оклейка плёнкой. Устроена как фриз — тянется в длину, несёт надпись,
   * ставится и снаружи стенда, — но без ограничения на число знаков
   * и считается в спецификации отдельно.
   */
  film?: boolean;
  /**
   * Ковёр: прямоугольник покрытия с шириной и глубиной, которые задают на месте.
   * Если на площадке есть ковёр, цветом ковра закрашивается только он,
   * а не вся площадка. Считается в спецификации в м².
   */
  carpet?: boolean;
...
export type Specification = {
  ...
  /** Суммарная длина оклейки плёнкой, метры. */
  filmLengthM: number;
  totalRub?: number;
...
export type FloorPlan = {
  ...
  /**
   * На сколько четвертей оборота против часовой стрелки площадку стенда
   * развернули относительно стенда на карте. Нужен, чтобы контур
   * непрямоугольного стенда поворачивался вместе с расстановкой.
   */
  turns?: number;
};
...
export type FurnitureObjectMeta = {
  ...
  rotation: number;
  /**
   * Надпись на фризовой панели. Пусто — берётся из названия сделки до слова
   * «стенд»: так надпись сама обновится, если сделку переименуют.
   */
  label?: string;
};
